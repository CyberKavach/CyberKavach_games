...
<?php
    ...
    $sql = "UPDATE `USERS` SET `UPGD` = 'T' WHERE `USERS`.`USERNAME` = '$ok'";
    $result = mysqli_query($conn,$sql); 
}
    $ko = $_SESSION['Points'];
...
{
    $YUG = $_POST["Score"];
    $sql = "UPDATE `USERS` SET `DVSCORE` = '$YUG' WHERE `USERS`.`USERNAME` = '$ok'";
    $result = mysqli_query($conn,$sql);
}
    $i=0;
    $ok=$_SESSION['name'];
    $sql= "SELECT * FROM `USERS` WHERE `USERS`.`USERNAME`='$ok' AND `USERS`.`DVSCORE`!='0'";
    $result = mysqli_query($conn,$sql);
    while($row = mysqli_fetch_assoc($result)){
        $i++;
        $cc=$row['DVSCORE'];
...
<body>
	...
	<nav class="navbar navbar-expand-lg navbar-light navbar-custom">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav mr-auto">
                ...
                <li class="nav-item">
                    <a class="nav-link" href="News.php" style="font-size:19px;padding: 16px 19px;font-family:Raleway">Cyber News</a>
                </li>
                ...
                <li class="nav-item">
                    <a class="nav-link" href="Palbearers.php" style="font-size:19px;padding: 16px 19px;font-family:Raleway">Cyber Upholders</a>
                </li>
                <li class="nav-item">
    ...
    <br>
    <h1 style="padding-left:50px"><ins>Dodge the Virus</ins></h1>
    <br>
<br> 
...
<div class="container">
    <div class="row">
        <div class="col-sm-1">
             ...
             <div class="col-sm-10">
    <iframe src="https://scratch.mit.edu/projects/423500614/embed" allowtransparency="true" width="855" height="690" frameborder="0" scrolling="no" allowfullscreen></iframe>
    </div>
...
<div class="container">
<div class="jumbotron" style="background-color: #fffd99">
<? if ($cc == 0) :?>
  <form action="DodgeVirus.php" class="form-inline" method="POST">

<div class="col-sm-6">
  <h3 style="font-family: Raleway">Share your score with us</h3>
  <input name="Score" type="number" id="quantity" name="quantity" min="0" max="60" class="form-control" placeholder="Score" style="width:100px">
...
<?php endif?>
<?php if ($cc != 0) :?>
<h3 style="font-family: Raleway"> Thank you for submitting your score</h3>
<h4 style="font-family: Raleway"> Your score is <?php echo $cc;?></h4>
<?php endif?>
</div>
</div>
<div class="container">
...
<table class="table table-bordered table-warning">
  ...
  <?php
  $i=1;
    $sql = "SELECT USERNAME, DVSCORE,NAME FROM USERS ORDER BY DVSCORE DESC LIMIT 5";
$result = mysqli_query($conn, $sql);


  
  while($row = mysqli_fetch_assoc($result) and $row["DVSCORE"] != 0) {
    if (strlen($row["NAME"])==0){
    echo "<tr>","<td>",$i,"</td>","<td>",$row["USERNAME"],"</td>","<td>",$row["DVSCORE"],"</td>","</tr>";
    $i++;
    }
    else {
echo "<tr>","<td>",$i,"</td>","<td>",$row["NAME"],"</td>","<td>",$row["DVSCORE"],"</td>","</tr>";
    $i++;
    }
