...
<?php
    ...
    $sql = "UPDATE `USERS` SET `UPGG` = 'T' WHERE `USERS`.`USERNAME` = '$ok'";
    $result = mysqli_query($conn,$sql); 
}
    $ko = $_SESSION['Points'];
...
{
    $YUG = $_POST["Score"];
    $sql = "UPDATE `USERS` SET `CFSCORE` = '$YUG' WHERE `USERS`.`USERNAME` = '$ok'";
    $result = mysqli_query($conn,$sql);
}
    $i=0;
    $ok=$_SESSION['name'];
    $sql= "SELECT * FROM `USERS` WHERE `USERS`.`USERNAME`='$ok' AND `USERS`.`CFSCORE`!='0'";
    $result = mysqli_query($conn,$sql);
    while($row = mysqli_fetch_assoc($result)){
        $i++;
        $cc=$row['CFSCORE'];
	...
	<title>Cyber-Games Cyber Frog</title>
...
<body>
    ...
    <br>
    <h1 style="padding-left:50px"><ins>Cyber Frog</ins></h1>
    <br>
<br> 
...
<div class="container">
    <div class="row">
        <div class="col-sm-1">
             ...
             <div class="col-sm-10">
    <iframe src="https://scratch.mit.edu/projects/422453501/embed" allowtransparency="true" width="855" height="690" frameborder="0" scrolling="no" allowfullscreen></iframe>
    </div>
...
<? if ($cc == 0) :?>
  <form action="CyberFrog.php" class="form-inline" method="POST">

<div class="col-sm-6">
  <h3 style="font-family: Raleway">Share your score with us</h3>
  <input name="Score" type="number" id="quantity" name="quantity" min="-20" max="6" class="form-control" placeholder="Score" style="width:100px">
...
<table class="table table-bordered table-warning">
  ...
  <?php
  $i=1;
    $sql = "SELECT USERNAME, CFSCORE,NAME FROM USERS ORDER BY CFSCORE DESC LIMIT 5";
$result = mysqli_query($conn, $sql);


  
  while($row = mysqli_fetch_assoc($result) and $row["CFSCORE"] != 0) {
    if (strlen($row["NAME"])==0){
    echo "<tr>","<td>",$i,"</td>","<td>",$row["USERNAME"],"</td>","<td>",$row["CFSCORE"],"</td>","</tr>";
    $i++;
    }
    else {
echo "<tr>","<td>",$i,"</td>","<td>",$row["NAME"],"</td>","<td>",$row["CFSCORE"],"</td>","</tr>";
    $i++;
    }
